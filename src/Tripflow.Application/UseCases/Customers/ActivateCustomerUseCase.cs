...
public class ActivateCustomerUseCase(
    ...
    ILogger<ActivateCustomerUseCase> logger) : IActivateCustomerUseCase
{
    public string ClassName = nameof(ActivateCustomerUseCase);
    ...
    public async Task<Result<CustomerResponse?>> ExecuteAsync(Guid id, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            logger.LogInformation("{ClassName} | {Method} | Cliente ativado com sucesso | {Id}", ClassName, Method, id);
        ...
        {
            logger.LogError("{ClassName} | {Method} | Erro inesperado ao ativar cliente | Id={Id} | {Message}",
                ClassName, Method, id, ex.Message);

            return Result<CustomerResponse?>.Failure("Erro inesperado ao ativar cliente.");
        }
    }
}
