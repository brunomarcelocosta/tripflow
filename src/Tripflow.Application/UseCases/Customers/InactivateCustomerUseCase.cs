...
public class InactivateCustomerUseCase(
    ICustomerRepository repository,
    ITravelerRepository travelerRepository,
    IUserContext userContext,
    ITenantContext tenantContext,
    IUserPermissionService userPermissionService,
    ILogger<InactivateCustomerUseCase> logger) : IInactivateCustomerUseCase
{
    public string ClassName = nameof(InactivateCustomerUseCase);
    ...
    public async Task<Result<CustomerResponse?>> ExecuteAsync(Guid id, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            customer.Inactivate(updatedBy);

            await repository.UpdateAsync(customer, cancellationToken);

            logger.LogInformation("{ClassName} | {Method} | Cliente inativado com sucesso | {Id}", ClassName, Method, id);
        ...
        {
            logger.LogError("{ClassName} | {Method} | Erro inesperado ao inativar cliente | Id={Id} | {Message}",
                ClassName, Method, id, ex.Message);

            return Result<CustomerResponse?>.Failure("Erro inesperado ao inativar cliente.");
        }
    }
}
