...
public class InactivateTenantPaymentProviderUseCase(
    ITenantPaymentProviderRepository repository,
    IUserContext userContext,
    ITenantContext tenantContext,
    IUserPermissionService userPermissionService,
    ILogger<InactivateTenantPaymentProviderUseCase> logger) : IInactivateTenantPaymentProviderUseCase
{
    public async Task<Result<bool>> ExecuteAsync(Guid id, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            entity.Inactivate(updatedBy);
            await repository.UpdateAsync(entity, cancellationToken);
            return Result<bool>.Ok(true);
        }
        catch (Exception ex)
        {
            logger.LogError("InactivateTenantPaymentProviderUseCase | Erro | {Message}", ex.Message);
            return Result<bool>.Failure("Erro inesperado ao inativar provedor de pagamento.");
        }
    }
}
