...
public class CreateQuotePricingOptionUseCase(
    ...
    IValidator<CreateQuotePricingOptionRequest> validator,
    IMapper mapper,
    IUserContext userContext,
    ITenantContext tenantContext,
    IUserPermissionService userPermissionService,
    ILogger<CreateQuotePricingOptionUseCase> logger) : ICreateQuotePricingOptionUseCase
{
    public async Task<Result<QuotePricingOptionResponse?>> ExecuteAsync(Guid quoteId, CreateQuotePricingOptionRequest request, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var option = new QuotePricingOption(
                tenantId, quoteId, request.Name,
                request.AgencyCost,
                request.DesiredProfitAmount,
                request.DesiredProfitPercentage,
            ...
            await pricingRepository.AddAsync(option, cancellationToken);

            if (request.AutoGeneratePaymentConditions)
            {
                var conditions = await calculatorService.GeneratePaymentConditionsAsync(option, createdBy, cancellationToken);
        ...
        {
            logger.LogError("CreateQuotePricingOptionUseCase | Erro | {Message}", ex.Message);
            return Result<QuotePricingOptionResponse?>.Failure("Erro inesperado ao criar opção de precificação.");
        }
    }
}
